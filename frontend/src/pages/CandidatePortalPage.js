...
export default function CandidatePortalPage() {
  ...
  const apply = async () => {
    ...
    try {
      await api.post('concours/dossiers/', { candidat: candidatId, concours: selectedConcours, reference });
      const resD = await api.get(`concours/dossiers/?candidat_id=${candidatId}`);
      setDossiers(resD.data);
      setStatus('Dossier soumis');
    } catch (e) {
      setStatus('Erreur de soumission');
    }
  };
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1em',marginTop:'1em'}}>
          <div style={{background:'#fff',borderRadius:12,boxShadow:'0 8px 24px #0001',padding:'1em'}}>
            ...
            <div style={{display:'flex',gap:'0.75em',alignItems:'center'}}>
              <select value={selectedConcours} onChange={e=>setSelectedConcours(e.target.value)} style={{flex:1,padding:'0.7em',border:'1px solid #e4e7ec',borderRadius:10}}>
                <option value="">Sélectionner...</option>
                {concours.map(c => (<option key={c.id} value={c.id}>{c.nom} — {c.frais_inscription} FCFA</option>))}
              </select>
              <button onClick={apply} style={{padding:'0.8em 1.2em',background:'#0d6efd',color:'#fff',border:'none',borderRadius:10,fontWeight:700}}>Soumettre</button>
            </div>
            ...
            <div style={{display:'grid',gap:'0.5em'}}>
              {dossiers.map(d => (
                <div key={d.id} style={{border:'1px solid #e4e7ec',borderRadius:10,padding:'0.7em'}}>
                  <div>Concours #{d.concours}</div>
                  <div>Statut: {d.statut}</div>
                  <div>Référence: {d.reference || '—'}</div>
                </div>
              ))}
