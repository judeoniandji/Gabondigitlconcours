...
class SerieViewSet(viewsets.ModelViewSet):
    ...
    def classement(self, request, pk=None):
        ...
        # Optimisation: Agréger les scores directement dans la base de données
        # pour éviter de charger toutes les notes en mémoire et de faire les calculs en Python.
        # Cela réduit la charge sur l'application et est beaucoup plus performant
        ...
        classement_query = Note.objects.filter(
            ...
            moyenne=ExpressionWrapper(
                F('total_points') / float(coeff_sum),
                output_field=FloatField()
            )
        ).order_by('-moyenne')

        # Formattage du résultat pour correspondre à la sortie attendue.
        classement = [
            {
